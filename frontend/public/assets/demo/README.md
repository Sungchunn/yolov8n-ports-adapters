...
videos and poster thumbnails there instead of relying on filesystem directory
listing.
...
browser-friendly MP4 files in `videos/`, writes poster JPEGs in `thumbnails/`,
and rewrites `videos.json`. The default seed is fixed so repeated runs are
reproducible; pass `--seed` to rotate the sample set.
