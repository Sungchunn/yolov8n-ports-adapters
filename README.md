...
Demo videos are committed under `assets/demo/videos/`. They are synthetic assets
generated by `scripts/generate_demo_videos.py`, so the repository can show default
videos without redistributing third-party traffic-camera footage. Full downloaded
datasets should stay local and are ignored by Git.
...
```bash
curl -X POST http://127.0.0.1:8000/v1/detect/upload \
  -F "file=@assets/demo/videos/synthetic-medium.mp4;type=video/mp4"
```
...
For a frontend demo, use `assets/demo/videos.json` as the source list for a random
video rotator instead of reading directories at runtime.
